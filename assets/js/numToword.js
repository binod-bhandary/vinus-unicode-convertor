    ...
    en: {
        computeHundredsPlaceWord: digit => {
            ...
            // 100, 101, 1001 ....
                ...
                return hundreds + (tens !== "" ? "and " + tens : "") + " Rupees";
            }
        },
    ...
    np: {
        computeHundredsPlaceWord: digit => {            
            ...
            if (len(digit) <= 2) return DATA[lang].DIGITS[digit];
                ...
                const tens = DATA[lang].DIGITS[parseInt(str(digit)[1] + str(digit)[2])];
                return hundreds + " सय " + tens + " रुपैंया";
            }
        },
...
const DECIMAL_MAPPER = {
    en: {
        computeDecimalPlaceWord: digit => {
            if (len(digit) === 0 || digit === 0) return "";
            else {
                if (digit <= 19) return DATA[lang].DIGITS[parseInt(digit)] + " Paisa";
                // 20 ... 99
                else if (digit >= 20 && digit <= 99) {
                    // 20, 30 ....
                    if (digit % 10 === 0) return DATA[lang].DIGIT_TENS[str(digit)[0]] + " Paisa";
                    // 20 ... 99
                    else
                        return (
                            DATA[lang].DIGIT_TENS[str(digit)[0]] +
    ...
    np: {
        computeDecimalPlaceWord: digit => {        
            if (len(digit) === 0 || digit === 0) return "";
                ...
                var num = parseInt(digit.substring(0, 2));
                return DATA[lang].DIGITS[num]  + " पैसा";
            } 
        }
    }
};
...
function computeDecWord(digit) {
    return DECIMAL_MAPPER[lang].computeDecimalPlaceWord(digit);
}

// entry point
function mapToWords(number, language = "np") {
    
    if (len(number) > 13 || isNaN(number)) return "Invalid";
    ...
    if (-1 != number.indexOf(".")){
        ...
        intNum = number_parts[0];
        decNum = number_parts[1];
    }

    let parsedNum = Math.abs(parseInt(intNum));   
    // direct mapping
    if (lang === "en") {
    ...
    if(decNum > 0) {
        splited.push(computeDecWord(decNum.substring(0, 2)));
    }

    var data ={
        // result : splited.filter(n => n.length > 0).join(", ")
        result : splited.filter(n => n.length > 0).join(" ")
    };
    return data;
}
