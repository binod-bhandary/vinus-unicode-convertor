...
var uD = require('./evaldU.js');
var sW = require('./evalsW.js');
// var uD = new Object;
// var sW = new Object;
var cR = new Object;
var iW = new Object;
...
const smartTranslateWords = function (values) {
...
const translateWords = function (sent, html) {
...
function hasSW(s) {
    var sIndex;
    if (sW["smartconverter_on"] == "true")
        if (sW[s] || sW[s.replace(/[aA]$/, "")] || sW[s + "a"]) return true;
    for (sIndex = s.length - 2; sIndex >= 0; sIndex--) {
        var _indexS = s.substring(sIndex);
        if (sW[_indexS] || sW[(_indexS.charAt(0).toLowerCase() + _indexS.substring(1))] || sW[(_indexS.charAt(0).toUpperCase() + _indexS.substring(1))] || sW[_indexS.toLowerCase()]) return true;
        else if (sW[_indexS.replace(/[b|w]/g, "v")] || sW[_indexS.replace(/[v|w]/g, "b")] || sW[_indexS.replace(/[v|b]/g, "w")] || sW[_indexS.replace(/sh/g, "s")] || sW[_indexS.replace(/Sh/g, "s")] || sW[_indexS.replace(/s/g, "sh")]) /*va = ba = wa; sha = sa */ return true;
        else if (sW[_indexS.replace(/bh/g, "v")] || sW[_indexS.replace(/v/g, "bh")] || sW[_indexS.replace(/ph/g, "f")] || sW[_indexS.replace(/f/g, "ph")]) return true;
        else if (sW[_indexS.replace(/ye/g, "e")] || sW[_indexS.replace(/e/g, "ye")] || sW[_indexS.replace(/aa/g, "a")] || sW[_indexS.replace(/a/g, "aa")]) return true;
        else if (sW[_indexS.replace(/ee/g, "i")] || sW[_indexS.replace(/i/g, "ee")] || sW[_indexS.replace(/oo/g, "u")] || sW[_indexS.replace(/u/g, "oo")]) return true;
        else if (sW[_indexS.replace(/ksh/g, "ch")] || sW[_indexS.replace(/chh/g, "ch")]) return true;
    }
    return false;
}
...
const translate = function (source, destination, html, smart) {
...
function convert(sent) {
    if (sW["smartconverter_on"] == "true") sent = sent.replace(/\s*\./g, " .").replace(/\s*\?/g, " ?");
        ...
        for (i = 0; i < words.length; i++) {
            words[i] = words[i].replace(/ri\^/g, "ari^");
            if (sW["smartconverter_on"] == "true") {
                // var wordsLen = words[i].replace(/[\;\,\'\",\?\.\-\+\=\&\!:\(\)\*\|\\]*$/, '');
                if (hasSW(words[i]));
                else if (iW[words[i]]) words[i] = iW[words[i]];
                else if (words[i].length > 3) {
                    var ec_0, ec_1, ec_2, ec_3, w__len;
                    ec_0 = words[i].charAt(words[i].length - 1).toLowerCase();
                    ec_1 = words[i].charAt(words[i].length - 2).toLowerCase();
                    ec_2 = words[i].charAt(words[i].length - 3).toLowerCase();
                    ec_3 = words[i].charAt(words[i].length - 4).toLowerCase();
            ...
            subs = words[i].split("/");
            for (j = 0; j < subs.length; j++)
                if (subs[j].length != 0) rVal += getAllUnicode(subs[j], false);
            rVal += " "
        }
        return rVal;
}
